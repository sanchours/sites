 ...
 * @step Проверяем наличие первого элемента результата поиска
 * @step Проверяем ссылки первого и второго элемента результата поиска
 * @step Проверяем отсутствие третьего элемента поиска
 */
...
hrefSecond = WebUI.getAttribute(findTestObject('Object Repository/CMS/Search/Result/a_first_result_goods'), 'href')

if (hrefSecond != BaseLink.getUrlDef() + '/' + DataCatalog.aliassection + '/' + DataCatalog.tecnicalname + '/' ) {
	throw new StepErrorException('Не та ссылка')
}

WebUI.verifyElementPresent(findTestObject('Object Repository/CMS/Search/Result/div_second_result_search'), 1)

hrefFirst = WebUI.getAttribute(findTestObject('Object Repository/CMS/Search/Result/a_second_result_goods'), 'href')

if (hrefFirst != BaseLink.getUrlDef() + '/' + DataSearch.aliassection + '/' ) {
	throw new StepErrorException('Не та ссылка')
}
