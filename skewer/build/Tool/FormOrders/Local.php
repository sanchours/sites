<?php

namespace skewer\build\Tool\FormOrders;

use skewer\build\Tool;
use skewer\components\forms\entities\FieldEntity;
use skewer\components\forms\service\FieldService;
use skewer\helpers\Download;
use yii\base\UserException;

/**
 * Модуль для скачивания файлов бэкапов (базы и файлов).
 */
class Local extends Tool\LeftList\ModulePrototype
{
    /** имя параметра "имя файла" */
    const keyFile = 'fileName';
    const keyForm = 'form';
    const keyField = 'field';

    /**
     * Метод - исполнитель функционала.
     * @param string $defAction
     */
    public function execute($defAction = '')
    {
        try {
            // попробовать загрузить файл
            $this->downloadFile();
        } catch (\Exception $e) {
            echo $e->getMessage();
        }
    }

    /**
     * Отдает файл.
     *
     * @throws UserException
     * @throws \ReflectionException
     * @throws \yii\web\NotFoundHttpException
     */
    private function downloadFile()
    {
        // имя файла
        $sFileName = basename($this->getStr(self::keyFile));
        // id field
        $iFieldId = $this->getInt(self::keyField);

        $fieldEntity = FieldEntity::getById($iFieldId);
        $service = new FieldService($fieldEntity->form_id);
        $fieldAggregate = $service->getField($iFieldId);

        // путь к файлу
        $sFilePath = PRIVATE_FILEPATH . $fieldAggregate->getPathToFormFiles() . $sFileName;

        if (!is_file($sFilePath)) {
            throw new UserException('file not found');
        }
        Download::protectedFile($sFilePath);

        // выйти
        exit;
    }
}
