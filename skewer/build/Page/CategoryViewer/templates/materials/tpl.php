...
use skewer\build\Page\CategoryViewer\templates\materials;
...
materials\Asset::register(Yii::$app->view, ['aActiveSections' => ArrayHelper::getColumn($list, 'id')]);

?>

<div class="b-category-4 <?php if (!empty($html_class)): ?><?= $html_class ?> <?php endif; ?>">
    <?php if ($title): ?>
        <h2><?= $title ?></h2>
    <?php endif; ?>
    <?php  foreach ($list as $aItem): ?>
        <div class="category__item section_<?=$aItem['id']; ?> category__item-<?=$aItem['designParams']['block;width_koef']; ?>x <?php if (!empty($aItem['designParams']['block;use_special_class'])):?>category__special<?php endif; ?>">
            ...
            <div class="category__title"><span><?=$aItem['title']; ?></span></div>
            ...
            <div class="category__wrap">
                ...
                <div class="category__link"><a href="<?=$aItem['href']; ?>"><?=Yii::t('CategoryViewer', 'read_more'); ?></a></div>
            </div>
        </div>
    <?php endforeach; ?>
</div>