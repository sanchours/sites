						...
						children: [
							...
							children: [
								...
								onChange: function() {
									...
									// Изменение поля вызвано открытием изображения в редакторе(вызов метода setValue в обработчике setup спровоцировал вызов onChange)
									if ( dialog.originalElement.afterSetup ){
										dialog.originalElement.afterSetup = false;
									} else {
										// Получить имя файла без расширения
										var match = newUrl.match(/[^\/]*(?=\.)/),
										    newAlt = (match !== null)? match[0] : "";
										dialog.getContentElement('info','txtAlt').setValue(newAlt);
									}
								...
								setup: function( type, element ) {
									if ( type == IMAGE ) {
										this.getDialog().originalElement.afterSetup = true;
