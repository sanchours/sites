...
namespace skewer\generators\adm_module;
...
class Generator extends \yii\gii\Generator
{
    ...
    {
        return 'Генератор adm модулей и представлений на основе AR. Слой Adm.';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['moduleName', 'fullNameAR', 'moduleTitle', 'moduleDescription'], 'required'],
    ...
    {
        return [
            'moduleName' => 'короткое имя модуля, например <code>News</code>, класс при этом будет <code>skewer\build\Adm\News</code>.',
    ...
    public function save($files, $answers, &$results)
    {
        ...
        if ($this->bInstall) {
            $api = new Api();
            $isInstall = $api->isInstalled($this->moduleName, Layer::ADM);
            if (!$isInstall) {
                $api->install($this->moduleName, Layer::ADM);
            }
    ...
    {
        return Yii::getAlias('@skewer/build/Adm/' . $this->moduleName);
    }
    ...
    public function validateModuleName()
    {
        ...
        $api = new Api();
        if (($this->bInstall) && ($api->isInstalled($this->moduleName, Layer::ADM))) {
