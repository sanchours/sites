...
Ext.onReady(function() {
    ...
    if(Ext.tree.plugin.TreeViewDragDrop){
        Ext.apply(Ext.tree.plugin.TreeViewDragDrop.prototype, {
            dragText : '{0} выбранный(ых) элемент(а){1}'
        });
    }

    if(Ext.grid.plugin.DragDrop){
        Ext.apply(Ext.grid.plugin.DragDrop.prototype, {
            dragText : '{0} выбранный(ых) элемент(а){1}'
        });
    }
    ...
    if(Ext.form.field.Time){
        Ext.apply(Ext.form.field.Time.prototype, {
            invalidText : "{0} не является правильным временем - время должно быть указана в формате {1}"
        });
    }
